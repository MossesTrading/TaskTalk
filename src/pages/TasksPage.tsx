  ...
  const load = useCallback(
    async (mode: 'first' | 'refresh') => {
      ...
      try {
        ...
        // Gunakan buildBoardsWithRepairFetch untuk mengambil riwayat lengkap unit
        // yang repair-nya dibuka sebelum rentang tanggal tapi ditutup di dalam rentang
        const next = await buildBoardsWithRepairFetch(rows, defs);
        setBoards(next);
      ...
      >
        ...
        <span className="task-card-quick">
          ...
          {board.jenis === 'PULL' ? (
            board.pullStats ? (
              <>
                <Pill label="Ditarik" value={board.pullStats.pulled} tone="fixed" />
                <Pill label="Sisa" value={board.pullStats.remaining} tone="repair" />
              </>
            ) : null
          ) : (
            <>
...
// ── Achievement PULL (pengambilan unit) ────────────────────────────────────── //
//
// Untuk task jenis PULL, hanya menampilkan quantity/unit per jam.
// Tidak ada repair, OK, atau pengerjaan repair.

function PullSection({ board }: { board: TaskBoard }) {
  ...
  return (
    <section className="panel-section repair-block">
      <header>
        <div>
          <h3>
            <Wrench size={16} /> Achievement pengambilan unit
          </h3>
          <small className="muted">
            Quantity/unit untuk task PULL. Grafik per jam ada di bagian "Achievement per jam" di atas.
          </small>
        </div>
      </header>
      ...
      {p && (
        <div className="global-stats">
          <small className="muted">
            <strong>Global (semua waktu):</strong> {p.total} unit · {p.pulled} sudah ditarik · {p.remaining} sisa ({p.pullRate.toFixed(1)}%)
          </small>
        </div>
      )}
      ...
      <div className="tiles">
        <Tile
          ...
          value={p?.total ?? 0}
          tone="ok"
          hint="global semua waktu"
        />
        <Tile
          label="Sudah ditarik"
          value={p?.pulled ?? 0}
          tone="fixed"
          hint={`sync = true · ${p?.pullRate.toFixed(1) ?? 0}%`}
        />
        <Tile
          label="Sisa"
          value={p?.remaining ?? 0}
...
function RepairSection({ board, periodLabel }: { board: TaskBoard; periodLabel: string }) {
  ...
  const g = board.globalRepairStats;
    ...
    <section className="panel-section repair-block">
      <header>
        <div>
          ...
          <small className="muted">
            Semua OK dari PIC REPAIR dalam rentang tanggal. Leadtime hanya dihitung untuk unit yang punya entri repair sebelum OK.
          </small>
        </div>
      </header>
      ...
      {g && (
        <div className="global-stats">
          <small className="muted">
            <strong>Global (semua waktu):</strong> {g.totalOpened} repair · {g.totalDone} selesai · {g.totalOpen} belum selesai ({g.globalDoneRate.toFixed(1)}%)
          </small>
        </div>
      )}
      ...
      {r.done === 0 ? (
        <p className="muted small-note">
          Tidak ada OK dari PIC REPAIR pada {periodLabel}.
        </p>
      ) : (
        <>
          <div className="tiles">
            <Tile
              label="OK dari PIC REPAIR"
              value={r.done}
              tone="fixed"
              hint={`achievement ${periodLabel}`}
            />
            <Tile
              label="Dengan repair sebelumnya"
              value={r.opened}
              tone="repair"
              hint={`bisa dihitung leadtimenya`}
            />
            <Tile
